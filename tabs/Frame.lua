...
function Soda.Frame:draw(exception)
  ...
  --  if self.shadowMesh then self.shadowMesh:draw() end
   -- self.mesh:draw()
    ...
    if self.shape then self:drawShape(sty)
    ...
    for _, v in ipairs(self.child) do
        --[[
        local ok, err = xpcall(function()  v:draw(exception) end, function(trace) return debug.traceback(trace) end) 
     if not ok then print(v.title, err) end
          ]]
        if v:draw(exception) then return true end
    end
